...
const MasterDataSourceName = "root:123456@tcp(39.107.77.94:3306)/superstar?charset=utf8"
...
func newEngin() *xorm.Engine {
	...
	engine.DumpAllToFile("./dump_user_info.sql")
	//engine.DumpAll() //接受一个io.Reader类型
	//engine.Import() //接受一个io.Writer类型
	//engine.ImportFile("./import_user_info.sql")
	return engine
}

func dbshow() {
	infor , _ := engine.DBMetas()
	fmt.Println("dbmetas:%s",infor)
	fmt.Printf("Tableinfo:", *engine.TableInfo(&UserInfo{}))
}
...
func ormOmitUpdate() {
	...
	if ok {
		if info.SysCreated > 0 {
			//在查询中排除sys_created字段，然后更新（sys_created该字段也是user_info表的字段）
			...
			//在查询中排除sys_created字段，然后更新(sys_created字段也是user_info表的字段)
			ok, _ := engine.ID(info.Id).Omit("sys_created").
